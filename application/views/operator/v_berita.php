...
<body class="hold-transition layout-top-nav layout-navbar-fixed">
                                ...
                                <div class="card-body">
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-5">
                                            ...
                                            <input type="hidden" name="idkegiatan" class="form-control form-control-sm">
                                        </div>
                                    </div>
                                    <div class="form-group row">
    ...
    <script>
        $(function() {
            ...
            const config_table = {
                url: '<?= site_url('operator/Berita/GetData') ?>',
                data: {
                    jeniskegiatan: "<?= $jenis ?>",
                    idkegiatan: <?= $_SESSION['npsn'] ?>
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(row) {
                            return '<center>' +
                                ' <button id="btn-ubah" class="btn btn-primary" data-id="' + row.id + '" data-toggle="tooltip" data-placement="top" title="Tombol Ubah">' +
            ...
            $('#tambah-baru').click(function() {
                ...
                $('#form-input').prop('action', "<?= site_url('operator/Berita/insert') ?>")
                $('input[name="jeniskegiatan"]').val('<?= $jenis ?>');
                $('input[name="idkegiatan"]').val(<?= $_SESSION['npsn'] ?>);
            ...
            $('#tabel-data').on('click', '#btn-ubah', function() {
                ...
                ajax_click(data, function data_ajax(r) {
                    ...
                    $('input[name="jeniskegiatan"]').val('Sekolah');
                    $('input[name="idkegiatan"]').val(<?= $_SESSION['npsn'] ?>);

                }, "<?= site_url('operator/Berita/GetDataByID') ?>", "<?= site_url('operator/Berita/update') ?>")
            });
            ...
            $('#tabel-data').on('click', '#btn-hapus', function() {
                ...
                hapus_data("<?= site_url('operator/Berita/delete') ?>", data)
            });
